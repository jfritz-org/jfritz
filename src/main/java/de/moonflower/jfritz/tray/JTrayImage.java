...
/**
 * Ermittelt anhand der zur Verfügung stehenden Größe in der Statusleiste des
...
public class JTrayImage
{
    ...
    public TrayIcon setIcon() throws IOException
    {
        try
        {
            ...
            {
                trayIconImage = ImageIO.read(loadResource("images/tray24.png"));
            }
            else if(trayIconSize.height > 16 && trayIconSize.height <= 22)
            {
                trayIconImage = scaleIconToBufferedImage(ImageIO.read(loadResource("images/tray22.png")),
            ...
            {
                trayIconImage = scaleIconToBufferedImage(ImageIO.read(loadResource("images/tray32.png")),
                    trayIconSize.width-1, trayIconSize.height-1, BufferedImage.TYPE_INT_ARGB);
            }
            else if(trayIconSize.height > 32 && trayIconSize.height <= 64)
            {
                trayIconImage = scaleIconToBufferedImage(ImageIO.read(loadResource("images/tray64.png")),
                    trayIconSize.width-1, trayIconSize.height-1, BufferedImage.TYPE_INT_ARGB);
            }
            else if(trayIconSize.height > 64 && trayIconSize.height <= 128)
            {
                trayIconImage = scaleIconToBufferedImage(ImageIO.read(loadResource("images/tray128.png")),
                    trayIconSize.width-1, trayIconSize.height-1, BufferedImage.TYPE_INT_ARGB);
            }
            else
            {
                trayIconImage = ImageIO.read(loadResource("images/tray16.png"));
            }
        ...
        catch(Exception exp)
        {
            ...
            trayIcon = new TrayIcon(ImageIO.read(loadResource("images/tray16.png")));
        }
